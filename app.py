...
def create_demo_interface(demo: VoxCPMDemo):
    ...
    with gr.Blocks(
        ...
        # Quick Start
        with gr.Accordion("📋 Quick Start Guide ｜快速入門", open=False, elem_id="acc_quick"):
            gr.Markdown("""
            ### How to Use ｜使用說明
            1. **(Optional) Provide a Voice Prompt** - Upload or record an audio clip to provide the desired voice characteristics for synthesis.  
               **（可選）提供參考聲音** - 上傳或錄製一段音頻，為聲音合成提供音色、語調和情感等個性化特徵
            2. **(Optional) Enter prompt text** - If you provided a voice prompt, enter the corresponding transcript here (auto-recognition available).  
               **（可選項）輸入參考文本** - 如果提供了參考語音，請輸入其對應的文本內容（支持自動識別）。
            3. **Enter target text** - Type the text you want the model to speak.  
               **輸入目標文本** - 輸入您希望模型朗讀的文字內容。
            4. **Generate Speech** - Click the "Generate" button to create your audio.  
               **生成語音** - 點擊"生成"按鈕，即可為您創造出音頻。
            """)

        # Pro Tips
        with gr.Accordion("💡 Pro Tips ｜使用建議", open=False, elem_id="acc_tips"):
            gr.Markdown("""
            ### Prompt Speech Enhancement｜參考語音降噪
            - **Enable** to remove background noise for a clean, studio-like voice, with an external ZipEnhancer component.  
              **啟用**：通過 ZipEnhancer 組件消除背景噪音，獲得更好的音質。
            - **Disable** to preserve the original audio's background atmosphere.  
              **禁用**：保留原始音頻的背景環境聲，如果想復刻相應聲學環境。

            ### Text Normalization｜文本正則化
            - **Enable** to process general text with an external WeTextProcessing component.  
              **啟用**：使用 WeTextProcessing 組件，可處理常見文本。
            - **Disable** to use VoxCPM's native text understanding ability. For example, it supports phonemes input ({HH AH0 L OW1}), try it!  
              **禁用**：將使用 VoxCPM 內置的文本理解能力。如，支持音素輸入（如 {da4}{jia1}好）和公式符號合成，嘗試一下！

            ### CFG Value｜CFG 值
            - **Lower CFG** if the voice prompt sounds strained or expressive.  
              **調低**：如果提示語音聽起來不自然或過於誇張。
            - **Higher CFG** for better adherence to the prompt speech style or input text.  
              **調高**：為更好地貼合提示音頻的風格或輸入文本。

            ### Inference Timesteps｜推理時間步
